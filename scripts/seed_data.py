...
def uri(base: str, key: str) -> URIRef:
    safe = key.replace(" ", "-").replace("/", "-").replace(".", "-").lower()
...
def add_station(g: Graph, station_id: str, comedian_uri: URIRef, door_uri: URIRef | None = None) -> URIRef:
...
def add_performance_record(g: Graph, r: dict, comedian_uri: URIRef, rec_date: date) -> URIRef:
...
def add_training_record(g: Graph, comedian_uri: URIRef, emp_id: str,
                         completed: str, expires: str, suffix: str = "-cert",
                         renewal_offered: bool = True) -> URIRef:
...
def add_wellbeing_pulse(g: Graph, emp_id: str, subject_uri: URIRef,
                        score: float, dimension: str, period: str = "2026-Q2") -> URIRef:
...
def add_cda_incident(g: Graph, uri_str: str, code: str, severity: int,
                      detected: str, reported: str,
                      door_uri: URIRef | None = None,
                      status: URIRef | None = None,
                      resolved: str | None = None,
                      responder: URIRef | None = None,
                      affected: URIRef | None = None) -> URIRef:
    node = URIRef(uri_str)
...
def main():
    ...
    for i, r in enumerate(valid_records):
        ...
        add_performance_record(g, r, comedian_uri, rec_date)
        record_count += 1
    ...
    g.add((inc_door, MI.lastMaintained, Literal((today - timedelta(days=20)).isoformat(), datatype=XSD.date)))
    ...
    add_cda_incident(g, inc2_uri, "2319", 3,
                     detected=dt(q1_2026, 9, 0), reported=dt(q1_2026, 9, 18),
                     door_uri=inc_door, status=MI.Closed, resolved=dt(q1_2026, 9, 55), responder=roz)
    ...
    add_cda_incident(g, inc3_uri, "2319", 5,
                     detected=dt(q4_2025, 10, 0), reported=dt(q4_2025, 11, 15),
                     door_uri=inc_door, status=MI.Escalated, resolved=dt(q4_2025, 12, 30),
    ...
    add_cda_incident(g, inc4_uri, "2319", 2,
                     detected=dt(q3_2025, 15, 30), reported=dt(q3_2025, 15, 45),
                     door_uri=inc_door, status=MI.SubmittedToRegulator,
                     resolved=dt(q3_2025, 16, 10), responder=roz)
    ...
    add_cda_incident(g, inc5_uri, "2319", 3,
                     detected=dt(open_recent, 11, 0), reported=dt(open_recent, 11, 12),
                     door_uri=inc_door, status=MI.Open, responder=roz)
    ...
    if phyllis:
        for com_id in ("EMP-011", "EMP-012", "EMP-013", "EMP-014", "EMP-015",
                       "EMP-016", "EMP-017", "EMP-018", "EMP-019", "EMP-023", "EMP-024"):
    ...
    table.add_row("mi:TrainingRecord",  str(comedian_count + 1), "1 expired (Randall), 1 expiring in 14d (George, Q5)")
    ...
    console.print("  1. [cyan]Randall Boggs (EMP-009)[/cyan] — assignedStation but TrainingRecord expired → ComedianCertShape")
    console.print("  2. [cyan]Door NYC-0099[/cyan] — lastMaintained 2023-09-01 (>180 days) → DoorDispatchShape")
    console.print("  3. [cyan]CDA incident (Q4 2025)[/cyan] — reported 75 min after detection (>30 min) → CDAReportingShape")
    console.print()
